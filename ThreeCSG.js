...
THREE.CSG = {
	toCSG: function ( three_model, offset, rotation ) {
		...
		rotation_matrix = new THREE.Matrix4( ).makeRotationFromEuler( new THREE.Euler(rotation.x, rotation.y, rotation.z) );
		...
		for ( i = 0; i < geometry.faces.length; i++ ) {
			if ( geometry.faces[i] instanceof THREE.Face3 ) {
				...
				var faceNormal = [ geometry.faces[i].normal.x, geometry.faces[i].normal.y, geometry.faces[i].normal.z ];
				vertices = [];
				vertices.push( new CSG.Vertex( geometry.vertices[geometry.faces[i].a].clone( ).add( offset ).applyProjection(rotation_matrix), faceNormal ) );
				vertices.push( new CSG.Vertex( geometry.vertices[geometry.faces[i].b].clone( ).add( offset ).applyProjection(rotation_matrix), faceNormal ) );
				vertices.push( new CSG.Vertex( geometry.vertices[geometry.faces[i].c].clone( ).add( offset ).applyProjection(rotation_matrix), faceNormal ) );
				...
				vertices.push( new CSG.Vertex( geometry.vertices[geometry.faces[i].a].clone( ).add( offset ).applyProjection(rotation_matrix), faceNormal ) );
				vertices.push( new CSG.Vertex( geometry.vertices[geometry.faces[i].b].clone( ).add( offset ).applyProjection(rotation_matrix), faceNormal ) );
				vertices.push( new CSG.Vertex( geometry.vertices[geometry.faces[i].d].clone( ).add( offset ).applyProjection(rotation_matrix), faceNormal ) );
				polygons.push( new CSG.Polygon( vertices ) );
	...
	fromCSG: function( csg_model ) {
		return this.fromCSGPolygons( csg_model.toPolygons() );
	},

	fromCSGPolygons: function( polygons ) {
