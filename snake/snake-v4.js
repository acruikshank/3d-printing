SNAKE = (function() {
  ...
  */
  ...
  var CYLINDER_QUALITY = 4*(PREPARE_FOR_PRINTING ? 32 : 4) + 1;
  ...
  var BODY_SPLINES = bundleSplines([
    snakeSection(-15, .1),
    ['curve',5,snakeSection(-15,.333*RADIUS1), snakeSection(-8,.666*RADIUS1), snakeSection(0,RADIUS1)],
  ...
  function renderAsync() {
    ...
    a3d.cad.init();
    var workers = [1,2,3,4,5,6,7,8].map(function() { return new Worker('snake-v4.js') });
  ...
  function renderSegment( startCenter, endCenter ) {
    ...
    M.lift(M.PathSource(splines, 20)).generate(
      M.facers( M.skin, M.closeEdge, M.capBottom, M.capTop )
    ).render(body.renderer);

    var segment = body.csgObject();

    if (startCenter > 0) {
      var renderer = M.CSGRenderer();

      var splines = socket(startCenter, RADIUS + RADIUS_EPSILON);

      M.lift(M.PathSource(splines, 20)).generate(
        M.facers( M.skin, M.closeEdge, M.capBottom, M.capTop )
      ).render(renderer.renderer);

      segment = segment.subtract(renderer.csgObject());
    }

    if (endCenter > 0) {
      var renderer = M.CSGRenderer();

      var splines = connector(endCenter, RADIUS + RADIUS_EPSILON);

      M.lift(M.PathSource(splines, 20)).generate(
        M.facers( M.skin, M.closeEdge, M.capBottom, M.capTop )
      ).render(renderer.renderer);

      segment = segment.union(renderer.csgObject());
    }
